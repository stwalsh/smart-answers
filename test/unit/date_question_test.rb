...
module SmartAnswer
  class DateQuestionTest < ActiveSupport::TestCase
    ...
    context '#parse_input' do
      setup do
        @question = Question::Date.new(:example)
      end
    ...
    test 'to_response reraises the exception if parse_input raises anything other than an InvalidResponse exception' do
      q = Question::Date.new(:example)
