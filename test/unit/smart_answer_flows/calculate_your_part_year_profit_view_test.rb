...
module SmartAnswer
  class CalculateYourPartYearProfitViewTest < ActiveSupport::TestCase
    ...
    context 'when rendering the result outcome' do
      ...
      context 'and the ceased_trading_on date is not set' do
        setup do
          @calculator.stubs(ceased_trading_on: nil)
          presenter = OutcomePresenter.new('i18n-prefix', @outcome, @state)
          @body = presenter.body(html: false)
        end
      ...
      context 'and the ceased_trading_on date is set' do
        setup do
          @calculator.stubs(ceased_trading_on: Date.parse('2016-04-05'))
          presenter = OutcomePresenter.new('i18n-prefix', @outcome, @state)
          @body = presenter.body(html: false)
        end
