  ...
  class CalculateYourPartYearProfitFlow < Flow
    def define
      ...
      date_question :when_did_you_stop_trading? do
        from { |state| state.calculator.tax_year.begins_on }
        to   { |state| state.calculator.tax_year.ends_on   }
        ...
        validate(:not_in_tax_year_error) do |response|
          calculator.valid_ceased_trading_date?(response)
        end

        next_node do |response|
          calculator.ceased_trading_on = response
